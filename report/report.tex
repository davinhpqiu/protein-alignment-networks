...
We may consider protein as a finite ordered sequence of amino-acid residues and
...
sequences alone. Typically protein sequences are very long and instead
we focus on protein domains - shorter, contiguous subsequences of a protein.
...
which pairs they report and is further discussed in Section \ref{sec:alignment_algo}

Pair scores are then used to define a sequence-similarity network (SSN) where vertices represent
sequences, and selected pairs form weighted edges. SSNs support exploration and
...
We then seek to ask how graphs built from different protein
collections, alignment algorithms, and graph construction rules compare and how well, if at all,
do these graphs recover known biological structure.

This study permutes those effects on a recorded panel of
\num{\MasterDomains} Pfam seed domains from \MasterFamilies\ families and
\MasterClans\ clans. A factorial design samples \num{\SampledCollections}
balanced collections; two scoring methods and 12 edge settings yield
\num{\GraphsEvaluated} weighted graphs. Community detection runs without
labels, and agreement with withheld Pfam family and clan labels quantifies
recovery.
...
Substitution matrix \(a(\cdot,\cdot)\) assigns score \(a(x_i,y_j)\) to an
...
\citep{smith1981identification}. Gotoh's formulation attains affine gap costs
in \(O(pq)\) time by carrying separate states for gapped and ungapped
extensions,
\begin{align*}
...
\]
Biopython's \texttt{PairwiseAligner} \citep{cock2009biopython} computed these
scores in local mode with BLOSUM62, \(g_o=11\), and \(g_e=1\).

\subsection{Heuristic search}
\label{ssec:heuristic}

The BLASTP algorithm avoids exhaustive dynamic programming. It indexes short words, locates
...
\citep{karlin1990methods}. Graph weights used \(S'\).

BLAST runs used BLOSUM62 and nominal gap costs 11 and 1, matching exact local
...
rather than from shared ancestry, for instance between two sequences both rich
in a single residue.

BLAST reports an alignment only when its expect value falls below a threshold.
For an alignment of raw score \(S\) and bit score \(S'\),
\[
...
Runs used \(E=10\) under standard BLAST default. That value is permissive by
...
\citep{llinareslopez2023deep}. Pretrained model was verified on pilot data.
Measured runtime exceeded available compute budget for all-versus-all scoring
of master panel, so scaled results use exact local alignment and BLAST only.

\section{Methods}

\subsection{Networks from pair scores}

For collection \(C\), let \(V\) be its vertex set, \(n=\lvert V\rvert\),
...
sequence remains a vertex; sequences with no selected pair become isolates.
...
Path from collection to reported quantity is
\[
...
withheld Pfam partition. Remaining subsections define each arrow. Master panel
...
The Pfam database defines families of homologous domains using curated seed alignments and
profile hidden Markov models; clans group families with evidence of common
ancestry \citep{mistry2021pfam}.

Panel selection follows a rejection sampling procedure over Pfam clans, run under seed
\SamplingSeed. The procedure first reads official family--clan catalogue, discards
unassigned families and clans holding fewer than three catalogue families, then
shuffles sorted clan list. Within each clan in that order it shuffles sorted
...
random stream drives every shuffle, so seed alone reproduces the panel. Trace
...
amino acids with consistent coordinates. Deterministic farthest-point sampling
then selects \TargetPerFamily\ representatives per family. First sequence has
...
For \(i\in V\), available neighbours are \(A_m(i)=\{j\in V:\{i,j\}\in A_m\}\). Let \(N_k^m(i)\) be the
first \(\min\{k,\lvert A_m(i)\rvert\}\) elements of
\(A_m(i)\) under descending score with ties broken by ascending identifier. For
\(e\in A_m\), let \(r_e^m\in(0,1]\) be ascending rank of \(s_e^m\) with ties
averaged and divided by \(\lvert A_m\rvert\) and \(q_e^m\) be descending rank
...
retains upper tail of scores a method reported, so it acts on \(A_m\); it
...
could not supply. Parameters are
\[
k\in\{1,2,5,10\},\qquad
...
Communities were detected using weighted Louvain optimisation
\citep{blondel2008fast}, as implemented in NetworkX
\citep{hagberg2008exploring}. This heuristic repeatedly moves vertices and
aggregates communities to increase weighted modularity
\[
...
\(W=\frac12\sum_{i,j}w_{ij}\). Parameter \(\gamma>0\) controls community
...
Choose one of \(n\) vertices uniformly. Let \(U\) be its detected community and
...
\]
with \(0\log0=0\); \(\log\) denotes natural logarithm. Normalised mutual information and adjusted mutual information are
\begin{align*}
...
settings whose community counts differ \citep{vinh2010information}. Both
metrics used scikit-learn \citep{pedregosa2011scikitlearn}. Family and clan
labels entered evaluation only after community detection.
...
Analysis centers around which edge rule yields communities closest to withheld Pfam
structure. Each Louvain partition was compared separately with family and clan
assignments. A tested setting is judged by median AMI across
\PrimaryCollections\ distinct primary collections; NMI shows effect of chance
correction. Detailed distribution plots use reference condition
...
\BioPercentileNinetyClanAMI, exceeding Biopython top-10 by 0.006; its BLAST
counterpart gave \BlastPercentileNinetyClanAMI. Top-10 was therefore first or
within 0.006 of first for both label levels and both score methods.
...
retain fewer edges; both median family AMI values exceed 0.91.
...
\begin{figure}[htbp]
  ...
  collections. Colour denotes rule family; marker denotes score method.}
...
percentile 0.90, median realised density was
...
\begin{figure}[htbp]
  ...
  conditions under top-5 construction. Colour gives clan count; panels separate
...
\begin{figure}[htbp]
  ...
  composition conditions. Rows separate score methods. Colour gives clan
  count; marker gives families per clan. Lines connect domain counts.}
  \label{fig:composition-topology}
\end{figure}

\subsection{Sensitivity analyses}

Score permutation within each collection preserved score distribution and rule
...
\begin{figure}[htbp]
  ...
  \caption{Sensitivity to score permutation and community weight.}
...
Top-\(k\) rules recovered withheld Pfam labels most consistently across tested
score methods and label levels. Top-10 maximised median family AMI for both
methods and was within 0.006 of highest clan AMI. Top-5 retained fewer edges
with median family AMI \BioTopFiveAMI\ for Biopython and \BlastTopFiveAMI\ for
BLAST. Selection between top-5 and top-10 depends on desired graph sparsity.

Edge allocation explains much of this result. Sparse global rules produced
many components; top-\(k\) produced fewer components at similar density because
each vertex selected local neighbours. Global density alone did not determine
graph structure.

NMI exceeded AMI under sparse, fragmented settings
(Figs.~\ref{fig:nmi-frequency} and~\ref{fig:ami-frequency}). AMI subtracts
expected mutual information conditional on observed group sizes. Comparisons
across rules and compositions therefore use AMI; NMI supplies an unadjusted
bounded descriptor.

Biopython and BLAST ranks agreed closely among pairs reported by BLAST, while
BLAST covered \BlastCoveragePct\% of master pairs. Equal percentiles combined
...
score distribution. Under unit weights, family AMI at reference setting was
\UnweightedTopFiveAMI, compared with \ObservedTopFiveAMI\ under native weights.
Controls locate observed signal primarily in edge placement for this setting.
...
communities, and auditable agreement measures. Among tested settings, top-\(k\)
rules recovered withheld Pfam structure most consistently; top-10 maximised
family AMI and top-5 supplied a sparser alternative with median family AMI
above 0.91 under both score methods. AMI corrected
